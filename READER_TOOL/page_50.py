...
global_current_word = {"label":"","roma":"","right":False}
def get_next():
    ...
    for i,item in enumerate(typing_list):
        if item['label'] == global_current_word['label']:
            ...
            break 
    curr_index +=1
    if curr_index < 0 or curr_index == len(typing_list):
        curr_index = 0
    label = typing_list[curr_index]["label"]
    roma  = typing_list[curr_index]['roma']
    global_current_word = {"label":label,"roma":roma,"right":False}

def check_input(e):
    if e.value == global_current_word['roma']:
        word = global_current_word['label']
        download_words(word)
        v = ui.audio(os.path.join(os.path.join(base_resource_dir,word+".mp3")))
        v.play()
        get_next()
        show_page.refresh()
...
def show_page():
    with ui.column().classes("w-full"):
        ...
        ui.input( ).props('autofocus').on_value_change(check_input)
@ui.page("/")
def page_50():
    
    global_typing_list['typing_list'] = get_data("all")
    get_next()
    show_page()
